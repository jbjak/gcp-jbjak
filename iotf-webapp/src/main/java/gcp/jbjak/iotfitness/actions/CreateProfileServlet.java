		...
		try {
			...
			if (user != null) {
				profile = ObjectifyService.ofy().load().type(Profile.class).filter("member_id", user.getUserId())
						.first().now();
				if (profile == null) {
					profile = new Profile(user.getUserId(), user.getEmail(), fname, lname, gender, birth_yr, height,
							weight, device_type);
				} else {
				...
				// Use Objectify to save the profile and now() is used to make the call
				// synchronously as we
				// will immediately get a new page using redirect and we want the data to be
				// present.
